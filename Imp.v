...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  match prog with
  | nil    => stack
  | SPush n :: xs => s_execute st (n::stack) xs 
  | SLoad x :: xs => s_execute st (st x :: stack) xs
  | SPlus   :: xs => match stack with 
                     | x :: y:: ss => s_execute st (x+y :: ss) xs
                     | _           => s_execute st stack xs
                     end
  | SMinus  :: xs => match stack with 
                     | x :: y:: ss => s_execute st (y-x :: ss) xs
                     | _           => s_execute st stack xs
                     end
  | SMult   :: xs => match stack with 
                     | x :: y:: ss => s_execute st (x*y :: ss) xs
                     | _           => s_execute st stack xs
                     end
  end. 
...
Check app_comm_cons.
Theorem app_assoc3 : forall {A:Type} (l m n k : list A),
(l++m++n) ++ k = l ++ (m++n++k).
Proof.
  intros A l. induction l.
  - simpl. intros m n k. rewrite app_assoc. reflexivity.
  - intros m n k. rewrite <- app_comm_cons. rewrite <- app_comm_cons.  rewrite -> IHl. rewrite <- app_comm_cons. reflexivity.
Qed.


Lemma s_compile_aux  : forall (st : state) (stack : list nat) (e : aexp) (si : list sinstr),
s_execute st stack ((s_compile e)++si) = s_execute st (aeval st e:: stack) si.
Proof.
  intros st stack e si. generalize dependent st. generalize dependent stack. generalize dependent si.
  induction e.
  - reflexivity.
  - reflexivity.
  - intros si stack st. simpl.  
  rewrite  (app_assoc3 (s_compile e1) (s_compile e2) ([SPlus]) (si)).
  rewrite IHe1. rewrite IHe2. simpl. rewrite Nat.add_comm. reflexivity.
  - intros si stack st. simpl.  
  rewrite  app_assoc3.  rewrite IHe1. rewrite IHe2. simpl. reflexivity.
  - intros si stack st. simpl.  
  rewrite  app_assoc3.  rewrite IHe1. rewrite IHe2. simpl. rewrite Nat.mul_comm. reflexivity.
Qed.
...
Proof. intros.  
rewrite <- (app_nil_r (s_compile e)). rewrite (s_compile_aux st [] e []).
reflexivity.
Qed.
